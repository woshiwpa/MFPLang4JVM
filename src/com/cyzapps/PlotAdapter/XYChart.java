// MFP project, XYChart.java : Designed and developed by Tony Cui in 2021
/**
...
public class XYChart extends FlatChart {
    ...
    public void draw(Graphics2D graphics, double x, double y, double width, double height) {
        ...
        // draw x-axis
        drawAxis(graphics, mcaXAxis, mp3CoordLeftBottomInTO.getX(), mp3CoordLeftBottomInTO.getY(),
                mp3CoordLeftBottomInTO.getX() + mdCoordWidth, mp3CoordLeftBottomInTO.getY(),
                mcolorForeGrnd, false);
        // draw y-axis
        drawAxis(graphics, mcaYAxis, mp3CoordLeftBottomInTO.getX(), mp3CoordLeftBottomInTO.getY(),
        ...
        graphics.drawLine((int) fCoordLeft, (int) fCoordTop, (int) fCoordRight, (int) fCoordTop);
        graphics.drawLine((int) fCoordRight, (int) fCoordTop, (int) fCoordRight, (int) fCoordBottom);
        ...
        drawDataCurves(graphics, mmapperP2P, mDataSet, mp3CoordLeftBottomInTO.getX(), mp3CoordLeftBottomInTO.getY() - mdCoordHeight,
                mdCoordWidth, mdCoordHeight);
        if (rectClip == null) {
    ...
    public void drawGrid(Graphics2D graphics) {
        ...
        for (int idx = 0; idx < mcaXAxis.marraydScaleMarks.length; idx++) {
            ...
            graphics.drawLine((int) fXMarksInTOX, (int) fXMarksInTOY, (int) fXMarksUpInTOX, (int) fXMarksUpInTOY);
        }
        ...
        for (int idx = 0; idx < mcaYAxis.marraydScaleMarks.length; idx++) {
            ...
            graphics.drawLine((int) fYMarksInTOX, (int) fYMarksInTOY, (int) fYMarksRightInTOX, (int) fYMarksRightInTOY);
        }
        graphics.setColor(colorPaintOriginalColor);
    }
    ...
    public void drawDataCurve(Graphics2D graphics, PointMapper mapper, DataSeriesCurve dsv, double x, double y, double width, double height) {
        java.awt.Color colorPaintOriginalColor = graphics.getColor();
        ...
        for (int idx = 0; idx < dsv.mlistData.size(); idx++) {
            ...
            /*Log.e(LOG_TAG, "in XYChart.findInRangeLine, x1 = " + x + " X2 = " + (x + width) + " y1 = " + y + " y2 = " + (y + height)
            ...
            if (p3Mapped.getX() >= x && p3Mapped.getX() <= (x + width)
                ...
                drawPoint(graphics, p3Mapped, dsv.mpointStyle);
            ...
            if (bShouldDrawLine) {
                drawLine(graphics, p3LnPnt0, p3LnPnt1, dsv.mlineStyle);
            }
    ...
    public void drawDataCurves(Graphics2D graphics, PointMapper mapper, LinkedList<DataSeriesCurve> dataSet, double x, double y, double width, double height) {
        for (int idx = 0; idx < mDataSet.size(); idx++) {
            drawDataCurve(graphics, mapper, dataSet.get(idx), x, y, width, height);
        }
    }
    ...
    public void drawButtons(Graphics2D graphics, double left, double top, double width, double height) {
        ...
        if (width <= height) {    // Portrait
            ...
            mrectZoomR.setRect((float) (left + width - dZoomButtonSize * 0.7),
        ...
        BufferedImage cfgImage, zoomInImage, zoomOutImage, xy1To1ZoomImage, fitZoomImage;
        ...
        if (width <= height) {    // Portrait
            ...
            dHShiftRatio++;
            graphics.drawImage(xy1To1ZoomImage, (int) (mrectZoomR.getX() + dZoomButtonSize * dHShiftRatio), (int) dButtonY, null);
            dHShiftRatio++;
            ...
            dVShiftRatio++;
            graphics.drawImage(xy1To1ZoomImage, (int) dButtonX, (int) (mrectZoomR.getY() + dZoomButtonSize * dVShiftRatio), null);
            dVShiftRatio++;
    ...
    public void drawLegends(Graphics2D graphics, boolean bPortrait) {
        ...
        graphics.setFont(new Font(graphics.getFont().getName(), graphics.getFont().getStyle(), (int) dTextSize));
        ...
        for (int idx = 0; idx < mDataSet.size(); idx++) {
            ...
            Position3D p3LineEnd2 = new Position3D(mp3LegendBoxLeftTop.getX() + dCvExampleWidth - dMarginSize,
                    dCurrentLegendStart + Math.max(dSize, dTextSize) / 2);
            drawPoint(graphics, p3Point, mDataSet.get(idx).mpointStyle);
            drawLine(graphics, p3LineEnd1, p3LineEnd2, mDataSet.get(idx).mlineStyle);
            if (bPortrait) {    // Portrait
                double dCvNameWidth = dLegendBoxWidth - 2 * dMarginSize;
                String strFittedName = getFitText(mDataSet.get(idx).mstrName, dCvNameWidth, graphics);
                draw1LineTextAnchorMid(graphics, strFittedName,
            ...
            if (dCurrentLegendStart >= (mp3LegendBoxLeftTop.getY() + mdCoordHeight * 2 / 3 - dMarginSize)
                ...
                draw1LineTextAnchorMid(graphics, "... ...",
        ...
        graphics.setColor(new java.awt.Color(mcolorForeGrnd.getF1R(), mcolorForeGrnd.getF1G(), mcolorForeGrnd.getF1B(), 64.0f / 256.0f)); // transparent background.
        ...
        graphics.drawRect((int) fLegendBoxLeft, (int) fLegendBoxTop, (int) (fLegendBoxRight - fLegendBoxLeft), (int) (fLegendBoxBottom - fLegendBoxTop));
