// MFP project, OGLChart.java : Designed and developed by Tony Cui in 2021
/**
...
import jcmdline.JMultiXYZChartAdj.AdjOGLChartParams;
...
public class OGLChart extends MFPChart implements GLEventListener, MouseListener, MouseMotionListener {
    ...
    protected LinearMapper mmapperP2P = new LinearMapper(); // overall mapper
    ...
    protected int mnPrevMouseX = -1, mnPrevMouseY = -1;
    ...
    public JMultiXYZChartAdj mMultiXYZChartAdj = null;    // the settings dialog box.
    ...
    public void clickCfgBtn() {
        if (mMultiXYZChartAdj == null) {
            mMultiXYZChartAdj = new JMultiXYZChartAdj((Frame) getGraphContainer(), true, this);
        }
        mMultiXYZChartAdj.resetParams();
        mMultiXYZChartAdj.setVisible(true);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getX() >= mrectSettingsBtnPos.getMinX() && e.getX() <= mrectSettingsBtnPos.getMaxX()
                && e.getY() >= mrectSettingsBtnPos.getMinY() && e.getY() <= mrectSettingsBtnPos.getMaxY()) {
            clickCfgBtn();
        } else if (e.getX() >= mrectZoom1BtnPos.getMinX() && e.getX() <= mrectZoom1BtnPos.getMaxX()
                && e.getY() >= mrectZoom1BtnPos.getMinY() && e.getY() <= mrectZoom1BtnPos.getMaxY()) {
            clickZoom1Btn();
        } else if (e.getX() >= mrectZoomFitBtnPos.getMinX() && e.getX() <= mrectZoomFitBtnPos.getMaxX()
                && e.getY() >= mrectZoomFitBtnPos.getMinY() && e.getY() <= mrectZoomFitBtnPos.getMaxY()) {
            clickZoomFitBtn();
        } else if (e.getX() >= mrectZoomInBtnPos.getMinX() && e.getX() <= mrectZoomInBtnPos.getMaxX()
                && e.getY() >= mrectZoomInBtnPos.getMinY() && e.getY() <= mrectZoomInBtnPos.getMaxY()) {
            clickZoomInBtn();
        } else if (e.getX() >= mrectZoomOutBtnPos.getMinX() && e.getX() <= mrectZoomOutBtnPos.getMaxX()
                && e.getY() >= mrectZoomOutBtnPos.getMinY() && e.getY() <= mrectZoomOutBtnPos.getMaxY()) {
            clickZoomOutBtn();
        }
    }
    ...
    public void mouseDragged(MouseEvent e) {
        int x = e.getX();
        int y = e.getY();
        if (mnPrevMouseX != -1 && mnPrevMouseY != -1) {   // if mnPrevMouseX and Y have been initialized.
            Dimension size = e.getComponent().getSize();

            double thetaY = Math.PI / 2 * ((float) (x - mnPrevMouseX) / (float) size.width);
            double thetaX = Math.PI / 2 * ((float) (y - mnPrevMouseY) / (float) size.height);
    ...
    public void init(GLAutoDrawable drawable) {
        mtextRenderer = new TextRenderer(new Font("SansSerif", Font.PLAIN, 72));
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        gl.glMatrixMode(GL2.GL_MODELVIEW);     //Select The Projection Matrix
    ...
    public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
        ...
        glu.gluPerspective(PROJECT_Y_OVERALL_ANGLE, (float) width / (float) height, PROJECT_Z_NEAR, PROJECT_Z_FAR);
        ...
        mnPrevMouseX = mnPrevMouseY = -1;
    }

    public void drawLine(GL2 gl, Position3D p3PrevPoint, Position3D p3NextPoint, com.cyzapps.VisualMFP.Color color) {
        ...
        gl.glDrawArrays(GL2.GL_LINES, 0, vertices.length / 3);

        // Disable the client state before leaving
        gl.glDisableClientState(GL2.GL_VERTEX_ARRAY);
    }

    public void drawRectangle(GL2 gl, Position3D p3LeftTop, Position3D p3RightBottom, com.cyzapps.VisualMFP.Color color) {
        /**
         * 3d chart ignore point shape and size. An alternative way is like
         * gl.glColor3f(r, g, b); gl.glBegin(GL.GL_QUADS);
         * gl.glVertex3f(-halfFaceSize, -halfFaceSize, halfFaceSize);
         * gl.glVertex3f( halfFaceSize, -halfFaceSize, halfFaceSize);
         * gl.glVertex3f( halfFaceSize, halfFaceSize, halfFaceSize);
         * gl.glVertex3f(-halfFaceSize, halfFaceSize, halfFaceSize); gl.glEnd();
         */
        gl.glColor4f(color.getF1R(), color.getF1G(), color.getF1B(), color.getF1Alpha());
        float vertices[] = {
        ...
        gl.glVertexPointer(3, GL2.GL_FLOAT, 0, vertexBuffer);
        // Draw the vertices as triangle strip
        gl.glDrawArrays(GL2.GL_TRIANGLE_STRIP, 0, vertices.length / 3);

        // Disable the client state before leaving
        gl.glDisableClientState(GL2.GL_VERTEX_ARRAY);
    }

    public void drawPoint(GL2 gl, Position3D p3Point, com.cyzapps.VisualMFP.Color color) {
        ...
        gl.glVertexPointer(3, GL2.GL_FLOAT, 0, vertexBuffer);
        // Draw the vertices as triangle strip
        gl.glDrawArrays(GL2.GL_POINTS, 0, vertices.length / 3);
    ...
    public void draw(GL2 gl) {
        ...
        try {
            if (mtextureSettingsBtn == null) {
                mtextureSettingsBtn = TextureIO.newTexture(getClass().getResource("/jcmdline/resources/setting_gear_64.png"), true, null);
            }
            if (mtextureZoom1Btn == null) {
                mtextureZoom1Btn = TextureIO.newTexture(getClass().getResource("/jcmdline/resources/zoom_1_64.png"), true, null);
            }
            if (mtextureZoomFitBtn == null) {
                mtextureZoomFitBtn = TextureIO.newTexture(getClass().getResource("/jcmdline/resources/zoom_fit_64.png"), true, null);
            }
            if (mtextureZoomInBtn == null) {
                mtextureZoomInBtn = TextureIO.newTexture(getClass().getResource("/jcmdline/resources/zoom_in_64.png"), true, null);
            }
            if (mtextureZoomOutBtn == null) {
                mtextureZoomOutBtn = TextureIO.newTexture(getClass().getResource("/jcmdline/resources/zoom_out_64.png"), true, null);
            }
        } catch (IOException ex) {
            Logger.getLogger(OGLChart.class.getName()).log(Level.SEVERE, null, ex);
        ...
        if (bInLandScapeMode) {  // landscape mode
            ...
            drawLegend(gl, pntLegendTopLeft, dLegendSize * 2, true); // dLegendSize * 2 is width, for Android it is 3.    
        ...
        Position3D pntSettingsTopLeft, pntZoom1TopLeft, pntZoomFitTopLeft, pntZoomInTopLeft, pntZoomOutTopLeft;
        // draw the zoom and settings buttons.
        pntSettingsTopLeft = new Position3D(
                -dChartTitleZ * Math.tan(PROJECT_Y_OVERALL_ANGLE * Math.PI / 360) * mdChartWindowWidth / mdChartWindowHeight - dButtonSize,
                pntChartTitle.getY() - dTitleSize, pntChartTitle.getZ());
        loadTexture(gl, mtextureSettingsBtn, pntSettingsTopLeft, dButtonSize, dButtonSize);
        pntZoom1TopLeft = new Position3D(
                -dChartTitleZ * Math.tan(PROJECT_Y_OVERALL_ANGLE * Math.PI / 360) * mdChartWindowWidth / mdChartWindowHeight - dButtonSize,
                pntChartTitle.getY() - dTitleSize - dButtonSize * 3 / 2, pntChartTitle.getZ());
        loadTexture(gl, mtextureZoom1Btn, pntZoom1TopLeft, dButtonSize, dButtonSize);
        pntZoomFitTopLeft = new Position3D(
                -dChartTitleZ * Math.tan(PROJECT_Y_OVERALL_ANGLE * Math.PI / 360) * mdChartWindowWidth / mdChartWindowHeight - dButtonSize,
                pntChartTitle.getY() - dTitleSize - dButtonSize * 3, pntChartTitle.getZ());
        loadTexture(gl, mtextureZoomFitBtn, pntZoomFitTopLeft, dButtonSize, dButtonSize);
        pntZoomInTopLeft = new Position3D(
                -dChartTitleZ * Math.tan(PROJECT_Y_OVERALL_ANGLE * Math.PI / 360) * mdChartWindowWidth / mdChartWindowHeight - dButtonSize,
                pntChartTitle.getY() - dTitleSize - dButtonSize * 9 / 2, pntChartTitle.getZ());
        loadTexture(gl, mtextureZoomInBtn, pntZoomInTopLeft, dButtonSize, dButtonSize);
        pntZoomOutTopLeft = new Position3D(
                -dChartTitleZ * Math.tan(PROJECT_Y_OVERALL_ANGLE * Math.PI / 360) * mdChartWindowWidth / mdChartWindowHeight - dButtonSize,
                pntChartTitle.getY() - dTitleSize - dButtonSize * 6, pntChartTitle.getZ());
        loadTexture(gl, mtextureZoomOutBtn, pntZoomOutTopLeft, dButtonSize, dButtonSize);
        ...
        mrectSettingsBtnPos = new Rectangle2D.Double(pntLeftTop.getX(), pntLeftTop.getY(),
                pntRightBottom.getX() - pntLeftTop.getX(), pntRightBottom.getY() - pntLeftTop.getY());
        ...
        mrectZoom1BtnPos = new Rectangle2D.Double(pntLeftTop.getX(), pntLeftTop.getY(),
                pntRightBottom.getX() - pntLeftTop.getX(), pntRightBottom.getY() - pntLeftTop.getY());
        ...
        mrectZoomFitBtnPos = new Rectangle2D.Double(pntLeftTop.getX(), pntLeftTop.getY(),
                pntRightBottom.getX() - pntLeftTop.getX(), pntRightBottom.getY() - pntLeftTop.getY());
        ...
        mrectZoomInBtnPos = new Rectangle2D.Double(pntLeftTop.getX(), pntLeftTop.getY(),
                pntRightBottom.getX() - pntLeftTop.getX(), pntRightBottom.getY() - pntLeftTop.getY());
        ...
        mrectZoomOutBtnPos = new Rectangle2D.Double(pntLeftTop.getX(), pntLeftTop.getY(),
                pntRightBottom.getX() - pntLeftTop.getX(), pntRightBottom.getY() - pntLeftTop.getY());

    }

    protected double calcTextWidth(GL2 gl, String text, double dHeight) {
        if (text == null) {
            return 0;
        } else {
            Rectangle2D bounds = mtextRenderer.getBounds(text);
            float w = (float) bounds.getWidth();
            float h = (float) bounds.getHeight();
            float fScalingFactor = (float) (dHeight / h);
            return w * fScalingFactor;
        }
    }

    protected String getLenLimitedString(GL2 gl, String text, double dHeight, double dLen) {
        if (text != null) {
            for (int idx = text.length(); idx >= 0; idx--) {
                String strNewText = text.substring(0, idx);
                if (idx == text.length() && calcTextWidth(gl, strNewText, dHeight) < dLen) {
                    return strNewText;
                } else if (calcTextWidth(gl, strNewText + "...", dHeight) < dLen) {
                    return strNewText + "...";
                }
            }
        }
    ...
    protected void drawZAdjustedText(GL2 gl, String text,
    ...
    protected void drawText(GL2 gl, String text,
            Position3D pntCentral, double dHeight, // width is determined by the text
            com.cyzapps.VisualMFP.Color color) {
        ...
        mtextRenderer.begin3DRendering();
        gl.glColor3f(color.getF1R(), color.getF1G(), color.getF1B());
        ...
        float w = (float) bounds.getWidth();
        float h = (float) bounds.getHeight();
        if (mgraphContainer != null && ((OGLChartView) mgraphContainer).getCanvas() != null) {
            FontMetrics metrics = ((OGLChartView) mgraphContainer).getCanvas().getFontMetrics(mtextRenderer.getFont());
            h = metrics.getHeight();
        }
        float fScalingFactor = (float) (dHeight / h);
        double dWidth = w * fScalingFactor;
        float x = (float) (pntCentral.getX() - dWidth / 2.0f);
        float y = (float) (pntCentral.getY() - dHeight / 2.0f);
        mtextRenderer.draw3D(text, x, y, (float) pntCentral.getZ(), fScalingFactor);
        mtextRenderer.flush();
        mtextRenderer.end3DRendering();
    }

    /**
     * convert a rectangle bitmap texture and apply it to a rectangle, note that
     * order of pnts is top left -> top right -> bottom left -> bottom right
     */
    public void applyRectTexture(GL2 gl, Position3D[] pnts, int nTextureId) {    //a 4 point rectangle.
        ...
        gl.glBindTexture(GL2.GL_TEXTURE_2D, mtextureMgr.mtextures[nTextureId]);
        ...
        gl.glDisable(GL2.GL_TEXTURE_2D);            //Disable Texture Mapping ( NEW )
    }

    /**
     * Draw an axis, assume at least one of dMarkOffsetX, Y, Z are not equal to
     * 0
     */
    public void drawAxis(GL2 gl, CoordAxis caAxis, Position3D pnt0, Position3D pnt1,
        ...
        for (int idx = 0; idx < caAxis.marraydScaleMarks.length; idx++) {
            ...
            if (caAxis.mdValueTo != caAxis.mdValueFrom) {
                ...
                double dTextWidth = calcTextWidth(gl, caAxis.marraystrMarkTxts[idx], dAxisFontSize);
                double dShiftedZ = 0;
                double dAdjMarkTxtZ = dZ - dMarkOffsetZ / dMarkOffsetLen * mdVerySmallSize - dShiftedZ;
                double dShiftedX = (dMarkOffsetX >= 0) ? dTextWidth / 2.0 : (-dTextWidth / 2.0);
                dShiftedX = dShiftedX / PROJECT_Z_FAR * -dAdjMarkTxtZ;
                double dShiftedY = (dMarkOffsetY >= 0) ? dAxisFontSize / 2.0 : (-dAxisFontSize / 2.0);
        ...
        double dNameTxtX = pnt1.getX(),
                dNameTxtY = pnt1.getY(),
                dNameTxtZ = pnt1.getZ();
        double dTextWidth = calcTextWidth(gl, caAxis.mstrAxisName, dAxisFontSize);
        double dShiftedZ = 0;
        ...
        Position3D pntNameTxt = new Position3D(dNameTxtX + dMarkOffsetX / dMarkOffsetLen * mdVerySmallSize + dShiftedX / PROJECT_Z_FAR * -dAdjNameTxtZ,
                dNameTxtY + dMarkOffsetY / dMarkOffsetLen * mdVerySmallSize + dShiftedY / PROJECT_Z_FAR * -dAdjNameTxtZ, dAdjNameTxtZ);

        drawZAdjustedText(gl, caAxis.mstrAxisName, pntNameTxt, dAxisFontSize, colorLine);
    }

    public void drawLegend(GL2 gl, Position3D pntLeftTop, double dWidthOrHeight, boolean bVerticalMode) {
            ...
            for (int idx = 0; idx < mDataSet.size(); idx++) {
                ...
                for (int idx1 = 0; idx1 < nNumberofLines; idx1++) {
                    ...
                    com.cyzapps.VisualMFP.Color colorCurrent = new com.cyzapps.VisualMFP.Color(
                            (colorMax.mnAlpha - colorMin.mnAlpha) / nNumberofLines * idx1 + colorMin.mnAlpha,
                            (colorMax.mnR - colorMin.mnR) / nNumberofLines * idx1 + colorMin.mnR,
                            (colorMax.mnG - colorMin.mnG) / nNumberofLines * idx1 + colorMin.mnG,
                            (colorMax.mnB - colorMin.mnB) / nNumberofLines * idx1 + colorMin.mnB);
    ...
    public void drawDataGridSurface(GL2 gl, DataSeriesGridSurface dsg) {
        ...
        if (dsg.msurfaceStyle.menumSurfaceType == SURFACETYPE.SURFACETYPE_GRID) {
            ...
            for (LinkedList<MultiLinkedPoint> itr : dsg.mlistSurfaceElements) {
                ...
                // Point to our vertex buffer
                gl.glColorPointer(4, GL2.GL_FLOAT, 0, colorBufferDownFace);  // Define color-array buffer
                // Draw the vertices as triangle strip
                //gl.glDrawArrays(GL2.GL_TRIANGLE_STRIP, 0, vertices.length / 3);
                gl.glDrawElements(GL2.GL_TRIANGLE_STRIP, indices.length, GL2.GL_UNSIGNED_BYTE, indexBuffer);

                gl.glCullFace(GL2.GL_BACK);
                gl.glColorPointer(4, GL2.GL_FLOAT, 0, colorBufferUpFace);  // Define color-array buffer
                gl.glDrawElements(GL2.GL_TRIANGLE_STRIP, indices.length, GL2.GL_UNSIGNED_BYTE, indexBuffer);

                gl.glDisable(GL2.GL_POLYGON_OFFSET_FILL);
                gl.glDisable(GL2.GL_CULL_FACE);
                gl.glDisableClientState(GL2.GL_COLOR_ARRAY);   // Disable color-array (NEW)
                gl.glDisableClientState(GL2.GL_VERTEX_ARRAY);
    ...
    public void drawDataGridSurfaces(GL2 gl) {
        for (int idx = 0; idx < mDataSet.size(); idx++) {
    ...
    void loadTexture(GL2 gl, Texture texture, Position3D p3LeftTop, double width, double height) {
        if (texture != null) {
            texture.enable(gl);
            texture.bind(gl);
            ...
            gl.glVertex3d(p3LeftTop.getX(), p3LeftTop.getY() - height, p3LeftTop.getZ());
            gl.glTexCoord2f(coords.right(), coords.bottom());
            gl.glVertex3d(p3LeftTop.getX() + width, p3LeftTop.getY() - height, p3LeftTop.getZ());
            gl.glTexCoord2f(coords.right(), coords.top());
            gl.glVertex3d(p3LeftTop.getX() + width, p3LeftTop.getY(), p3LeftTop.getZ());
            gl.glTexCoord2f(coords.left(), coords.top());
            gl.glVertex3d(p3LeftTop.getX(), p3LeftTop.getY(), p3LeftTop.getZ());
            gl.glEnd();
            texture.disable(gl);
        }
    }
