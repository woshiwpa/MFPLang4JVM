// MFP project, OGLExprChart.java : Designed and developed by Tony Cui in 2021
...
import jcmdline.J3DExprChartAdj.AdjOGLExprChartParams;
...
public class OGLExprChart extends OGLChart {
    ...
    public J3DExprChartAdj m3DExprChartAdj = null;    // the settings dialog box.
    ...
    public synchronized void applyCfgChart(AdjOGLExprChartParams adjParams) {
        ...
        if (!adjParams.isNoAdj(dXMin, dXMax, nXNumOfSteps, dYMin, dYMax, nYNumOfSteps, dZMin, dZMax, nZNumOfSteps, mbNotDrawAxis, mbNotDrawTitle) && !mbIsBusy) {
            if (!mbIsBusy) {
                mbIsBusy = true;
                getGraphContainer().setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
            }
    ...
    public void clickCfgBtn() {
        if (m3DExprChartAdj == null) {
            m3DExprChartAdj = new J3DExprChartAdj((Frame) getGraphContainer(), true, this);
        }
        m3DExprChartAdj.resetParams(getChartDataParams());
        m3DExprChartAdj.setVisible(true);
    }
    ...
    public synchronized void initialize() {
        ...
        if (!mbIsBusy) {
            mbIsBusy = true;
            //getGraphContainer() is still null
            //getGraphContainer().setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        }
    }
